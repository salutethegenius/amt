...
export async function POST(request: Request) {
  ...
  const secret = process.env.CNG_WEBHOOK_SECRET;
  const signature =
  ...
  const { data: session, error: sessionError } = await supabase
    ...
    .select("id, invoice_id, status, expected_amount_cents")
  ...
  let row = mapWebhookPayloadToRow(body, session.invoice_id);
  try {
    ...
    if (cngTx) {
      row = mapCngTransactionToRow(cngTx, { invoiceId: session.invoice_id });
    }
  } catch {
